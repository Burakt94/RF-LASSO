...
findsplit <- function(response, data, indices,mtry_control=NULL){
  
  candidates = get_features(mtry_control)[[1]]
  formula <- as.formula(paste0(response,"~",paste0(candidates,collapse = "+")))
  fit = rpart(formula,data=data[indices!=0],cp=-1,maxdepth = 1)
  best_split = tryCatch({
  ...
  return(partysplit(varid = as.integer(which(colnames(data)==best_split$var)),
                    breaks = best_split$index,
                    info = list(best=best_split$var,candidates=candidates)))
...
random_forest_custom_tree <- function(data,target,ntrees,minbucket=5,max_depth=4,bagging_ratio=0.632,
                                      mtry_control=NULL,sample_weights=NULL,bagging_type="random"){
  ...
  fits = mclapply(1:ntrees,function(n){
    ...
    fit = mytree(formula, data=data[in_index,], indices = NULL,
                 minbucket=minbucket,max_depth=max_depth,mtry_control = mtry_control,
                 feature_selection = "raw")
    fit$fit$data = head(fit$fit$data,1)
    return_fit = list(fit=fit$fit)
    rm(fit)
    gc()
    return(return_fit)
  },mc.cores = 2)
...
custom_tree_prediction <- function(fit_list,data,type=NULL){
  if(is.null(type)){
    preds = unlist(lapply(fit_list, function(x){predict(x$fit,data)}))
    ...
    preds = unlist(lapply(fit_list, function(x){
      nodes = predict(x$fit,data,type="node")
      ...
      setNames(matched, 1:length(matched))
    }))
...
predict_ranger <- function(fit,train_data,test_data,type="mean_mean",inbag_counts){
  train_nodes = predict(fit,train_data,type = "terminalNodes")$predictions
  test_nodes = predict(fit,test_data,type = "terminalNodes")$predictions
  
  if(type=="median_median"){
    preds = unlist(lapply(1:general_ntree, function(x){
      temp_pred_df = train_data %>% transmute(production,node = train_nodes[,x],inbags = inbag_counts[[x]]) %>%
        filter(inbags>0) %>%
        group_by(node) %>%
        summarise(median = median(production),mean=mean(production))
      nodes = test_nodes[,x]
      matched = temp_pred_df$median[match(nodes,temp_pred_df$node)]
      prediction = setNames(matched, 1:length(matched))
      return(prediction)
    ...
    preds = unlist(lapply(1:general_ntree, function(x){
      temp_pred_df = train_data %>% transmute(production,node = train_nodes[,x],inbags = inbag_counts[[x]]) %>%
        ...
        summarise(median = median(production),mean=mean(production))
      nodes = test_nodes[,x]
      matched = temp_pred_df$mean[match(nodes,temp_pred_df$node)]
      prediction = setNames(matched, 1:length(matched))
      return(prediction)
    }))
    pred_df = data.table(name=as.numeric(names(preds)),pred=preds) %>% group_by(name) %>% summarise(prediction = median(pred)) 
  }else if(type=="mean_mean"){
    preds = unlist(lapply(1:general_ntree, function(x){
      temp_pred_df = train_data %>% transmute(production,node = train_nodes[,x],inbags = inbag_counts[[x]]) %>%
        filter(inbags>0) %>%
        group_by(node) %>%
